...
function Field({ label, children }) {
  return (
    <label className="block">
      <span className="block text-[11px] font-medium tracking-[0.18em] uppercase text-[#1F4A2E]/55 mb-1.5">
        {label}
      </span>
      {children}
    </label>
...
const inputCls =
  "w-full bg-transparent border-b border-[#1F4A2E]/25 py-2 text-[#1F4A2E] placeholder:text-[#1F4A2E]/35 focus:border-[#A67C00] outline-none";
...
export default function Constructions() {
  ...
  return (
    <section id="construcoes" className="py-24 lg:py-32 bg-[#EAF0E5]">
      <div className="max-w-7xl mx-auto px-6 lg:px-10">
        ...
        <div className="space-y-10">
          {cases.map((c, i) => (
            <motion.article key={c.num} {...fade(i * 0.05)} className="grid lg:grid-cols-12 gap-6 lg:gap-10 border-t border-[#1F4A2E]/15 pt-8">
              <div className="lg:col-span-4">
                <span className="font-display text-4xl text-[#A67C00] leading-none">{c.num}</span>
                <p className="mt-3 text-[11px] font-medium tracking-[0.18em] uppercase text-[#1F4A2E]/50">{c.tag}</p>
                <h3 className="font-display text-xl lg:text-2xl text-[#1F4A2E] mt-2 leading-tight">{c.title}</h3>
              </div>
              <div className="lg:col-span-8 grid sm:grid-cols-2 gap-x-8 gap-y-5">
                <div>
                  ...
                  <p className="text-[#1F4A2E]/75 text-sm leading-relaxed">{c.contexto}</p>
                </div>
                <div>
                  <p className="text-[11px] font-medium tracking-[0.18em] uppercase text-[#A67C00] mb-1">{labels.desafio}</p>
                  <p className="text-[#1F4A2E]/75 text-sm leading-relaxed">{c.desafio}</p>
                </div>
                <div>
                  <p className="text-[11px] font-medium tracking-[0.18em] uppercase text-[#A67C00] mb-1">{labels.atuacao}</p>
                  <p className="text-[#1F4A2E]/75 text-sm leading-relaxed">{c.atuacao}</p>
                </div>
                <div>
                  <p className="text-[11px] font-medium tracking-[0.18em] uppercase text-[#A67C00] mb-1">{labels.diferencial}</p>
                  <p className="text-[#1F4A2E]/75 text-sm leading-relaxed">{c.diferencial}</p>
                </div>
                <div className="sm:col-span-2 border-t border-[#1F4A2E]/12 pt-4">
                  <p className="text-[11px] font-medium tracking-[0.18em] uppercase text-[#1F4A2E]/50 mb-1">{labels.resultado}</p>
                  <p className="text-[#1F4A2E] text-[15px] leading-relaxed font-medium">{c.resultado}</p>
                </div>
              </div>
            </motion.article>
          ))}
        ...
        <div className="mt-20 grid lg:grid-cols-12 gap-10 lg:gap-12 items-start">
          <div className="lg:col-span-5">
            <h3 className="font-display text-2xl lg:text-3xl text-[#1F4A2E] leading-tight">{t("constructions.formTitle")}</h3>
            <p className="mt-3 text-[#1F4A2E]/70 text-[15px] leading-relaxed">{t("constructions.formIntro")}</p>
          </div>
          <form onSubmit={submit} className="lg:col-span-7 grid gap-4 bg-[#F4EFEA] border border-[#C2D2C0]/70 rounded-2xl p-6 lg:p-8">
            ...
            <button
              type="submit"
              disabled={sending}
              className="mt-2 inline-flex items-center gap-2 self-start rounded-full bg-[#A67C00] px-6 py-2.5 text-sm font-medium tracking-[0.12em] uppercase text-[#F4EFEA] hover:bg-[#8A6300] transition-colors disabled:opacity-60"
            >
