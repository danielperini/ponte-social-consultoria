...
export default function Articles() {
  ...
  return (
    <section id="artigos" className="relative py-24 lg:py-40 bg-[#F6F6F6] overflow-hidden">
      <div className="max-w-7xl mx-auto px-6 lg:px-10">
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-10">
          ...
          <p className="text-[#073050]/70 max-w-sm text-[15px] leading-relaxed">
            {t("articles.intro")}
          </p>
        </div>

        <div className="relative max-w-xl mb-12">
          <Search size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-[#073050]/40" />
          <input
            type="text"
            value={query}
            ...
            className="w-full bg-[#A4B29B]/15 border border-[#A4B29B]/50 rounded-full pl-12 pr-11 py-3.5 text-[15px] text-[#073050] placeholder:text-[#073050]/40 outline-none focus:border-[#A67C00] transition-colors"
          />
          {query && (
            <button
              onClick={() => setQuery("")}
              aria-label="Clear"
              className="absolute right-4 top-1/2 -translate-y-1/2 text-[#073050]/40 hover:text-[#1B562A] transition-colors"
            >
              <X size={18} />
            </button>
          )}
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-20 text-[#073050]/60">
              ...
              >
                <Link to={`/artigos/${a.slug}`} className="group block h-full">
                  ...
                  <div className="mt-5">
                    ...
                    <p className="text-[#073050]/70 text-[15px] leading-relaxed">{a.excerpt}</p>
                    <span className="mt-4 block text-xs tracking-[0.12em] uppercase text-[#073050]/45">
                      {a.author}
                    </span>
                  </div>
                </Link>
