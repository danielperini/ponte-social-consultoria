      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-10">
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-display text-lg font-medium text-[#F4EFEA]">Ponte Social</p>
              <p className="text-xs text-[#F4EFEA]/50">A dimensão social dos negócios</p>
            </div>
          </div>
          <p className="font-display italic text-[#C87A53] text-lg font-light max-w-md">
            Porque relações também influenciam decisões, resultados e reputação.
          </p>
        </div>
        <div className="border-t border-[#F4EFEA]/10 mt-10 pt-6 flex flex-col sm:flex-row justify-between gap-3 text-xs text-[#F4EFEA]/40">
          <p>© {new Date().getFullYear()} Ponte Social. Todos os direitos reservados.</p>
          <p>Relações que conectam contextos à decisão.</p>
        </div>
      </div>
    </footer>
  );
